(() => {
  console.info('[Velora] live UI v15 loaded');
  ...
  async function loadChannels(force=false){if(!channelsPromise||force)channelsPromise=fetch(`/api/live?country=${encodeURIComponent(currentCountry)}&limit=500${force?'&refresh=1':''}`,{cache:'no-store'}).then(async r=>{const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||`Live API ${r.status}`);return Array.isArray(d.channels)?d.channels:[]});return channelsPromise;}
  async function loadSports(force=false){if(!sportsPromise||force)sportsPromise=fetch(`/api/live/upcoming${force?'?refresh=1':''}`,{cache:'no-store'}).then(r=>r.json()).then(x=>({events:Array.isArray(x.events)?x.events:[],focus:x.focus||{mode:'channels',label:'Best live channels',sport:null,events:[]}})).catch(()=>({events:[],focus:{mode:'channels',label:'Best live channels',sport:null,events:[]}}));return sportsPromise;}
  ...
  function eventPlayback(ev,channels){
    // v15: the featured Live & Upcoming rail is a trust surface. Only a channel
    // that has already passed Velora's HLS health check is allowed to become a
    // clickable live event. Streamed remains a schedule/discovery source, but an
    // embed existing is not proof that its underlying live manifest will play.
  ...
  function eventCard(ev,channels){
    ...
    const action=timing.phase==='live'?(playback?'Verified stream ready':'No verified stream yet'):timing.phase==='soon'?(playback?'Verified stream ready':'Stream check begins near start'):'Stream check begins near start';
    ...
    card.addEventListener('click',async()=>{
      ...
      try{openPlayer(matched,ev.shortName||ev.name)}finally{card.dataset.busy='0';card.classList.remove('resolving')}
    });
    return card;
  }
  ...
  function render(page,channels,sports){page.querySelectorAll('.veloraLiveInjected').forEach(x=>x.remove());page.querySelector('.empty,.liveLoadingPanel')?.remove();[...page.children].forEach(child=>{if(!child.classList.contains('veloraLiveInjected'))child.classList.add('veloraLiveOriginal')});page.classList.add('livePageEnhanced','duloLivePage');const verified=channels.filter(c=>c.health==='working'),down=channels.filter(c=>c.health==='down'),unknown=channels.filter(c=>c.health==='unknown'),good=verified;const focus=sports.focus||{mode:'channels',label:'Best live channels',events:[]};
    const now=Date.now();
    // Trust ESPN for event state/timing. Streamed-only entries are useful as a
    // source attachment layer, but its live list is not reliable enough to be
    // promoted into Velora's featured rail by itself.
    const scheduledEvents=(sports.events||[]).filter(ev=>{
      if(ev.stateSource==='streamed' || ev.provider==='Streamed') return false;
      const t=Date.parse(ev.date||'');const delta=t-now;
      if(!Number.isFinite(t)) return false;
      if(ev.state==='in') return Boolean(eventPlayback(ev,channels));
