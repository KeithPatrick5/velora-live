(() => {
  console.info('[Velora] live UI v12 loaded');
  ...
  function eventCard(ev,channels){const playback=eventPlayback(ev,channels);if(!playback)return null;const matched=playback.channel||null;const card=document.createElement('button');card.className='duloEventCard watchable '+(ev.state==='in'?'isLive':'isUpcoming');const fmt=ev.state==='in'?(ev.detail||'Live now'):(ev.date?new Date(ev.date).toLocaleString([],{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'Upcoming');const sourceLabel=playback.type==='streamed'?'Sports stream':matched?.name||'Live channel';card.innerHTML=`<span class="duloLeague">${ev.state==='in'?'● LIVE · ':''}${esc(ev.league||'LIVE')}</span><div class="duloMatchup"><div>${ev.away?.logo?`<img src="${esc(ev.away.logo)}" alt="">`:`<b>${esc(ev.away?.abbreviation||ev.away?.shortName?.slice(0,3)||'A')}</b>`}</div><i>vs</i><div>${ev.home?.logo?`<img src="${esc(ev.home.logo)}" alt="">`:`<b>${esc(ev.home?.abbreviation||ev.home?.shortName?.slice(0,3)||'H')}</b>`}</div></div><strong>${esc(ev.shortName||ev.name||'Live event')}</strong><small>${esc(fmt)}</small><em>${ev.state==='in'?'Watch live':'Stream scheduled'} · ${esc(sourceLabel)} →</em>`;card.addEventListener('click',async()=>{if(card.dataset.busy==='1')return;card.dataset.busy='1';card.classList.add('resolving');try{if(playback.type==='streamed'){const ok=await openStreamedEvent(ev);if(!ok&&matched){openPlayer(matched,ev.shortName||ev.name);return}if(!ok){card.classList.add('streamFailed');const em=card.querySelector('em');if(em){em.textContent='Stream unavailable right now';setTimeout(()=>{em.textContent=(ev.state==='in'?'Watch live':'Stream scheduled')+' · '+sourceLabel+' →';card.classList.remove('streamFailed')},2600)}}}else openPlayer(matched,ev.shortName||ev.name)}finally{card.dataset.busy='0';card.classList.remove('resolving')}});return card;}
  ...
  function heroHTML(slide,focus,good,verified){if(slide?.type==='event'){const ev=slide.event,pb=slide.playback;const ch=pb?.channel||null;const when=ev.state==='in'?(ev.detail||'Live now'):(ev.date?new Date(ev.date).toLocaleString([],{weekday:'short',hour:'numeric',minute:'2-digit'}):'Upcoming');const watchLabel=pb?.type==='streamed'?'Watch live':ch?`Watch on ${esc(ch.name)}`:'Browse sports channels';return `<div class="duloHeroCopy"><span class="duloHeroEyebrow">${ev.state==='in'?'● LIVE':'UP NEXT'} · ${esc(ev.league)}</span><h1>${esc(ev.shortName||ev.name)}</h1><p>${esc(when)}${ev.broadcasts?.length?` · ${esc(ev.broadcasts.slice(0,2).join(' / '))}`:''}</p><div class="duloHeroCounts"><b>${esc(focus.label||'Live sports')}</b> · ${good.length} channels · ${verified.length} verified</div><button class="duloChoose" data-action="watch">${watchLabel}</button></div><div class="duloHeroTeams">${ev.away?.logo?`<img src="${esc(ev.away.logo)}" alt="">`:''}${ev.home?.logo?`<img src="${esc(ev.home.logo)}" alt="">`:''}</div>`}const ch=slide?.channel;return `<div class="duloHeroCopy"><span class="duloHeroEyebrow">LIVE TV</span><h1>${esc(ch?.name||'Live TV')}</h1><p>${ch?'One of the best currently verified live channels.':'Watch live channels. New sources are checked continuously.'}</p><div class="duloHeroCounts"><b>${good.length}</b> channels · <b>${verified.length}</b> verified</div><button class="duloChoose" data-action="${ch?'watch':'channels'}">${ch?'Watch live':'Choose a channel'}</button></div><div class="duloHeroTeams">${ch?.logo?`<img src="${esc(ch.logo)}" alt="">`:''}</div>`;}

  function mountHero(shell,slides,focus,good,verified){const hero=shell.querySelector('.duloLiveHero');if(heroTimer)clearInterval(heroTimer);let idx=0;function draw(){const slide=slides[idx]||null;hero.innerHTML=heroHTML(slide,focus,good,verified)+`<div class="duloHeroDots">${slides.map((_,i)=>`<button aria-label="Slide ${i+1}" class="${i===idx?'active':''}" data-i="${i}"></button>`).join('')}</div>`;hero.querySelector('.duloChoose')?.addEventListener('click',async()=>{if(slide?.type==='event'){if(slide.playback?.type==='streamed'){await openStreamedEvent(slide.event);return}if(slide.playback?.channel){openPlayer(slide.playback.channel,slide.event?.shortName||slide.event?.name||'');return}}if(slide?.channel)openPlayer(slide.channel,slide?.event?.shortName||slide?.event?.name||'');else shell.querySelector('.duloRails')?.scrollIntoView({behavior:'smooth',block:'start'})});hero.querySelector('.duloHeroDots')?.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;idx=Number(b.dataset.i)||0;draw()})}draw();if(slides.length>1)heroTimer=setInterval(()=>{idx=(idx+1)%slides.length;draw()},6500);}
  ...
  function render(page,channels,sports){page.querySelectorAll('.veloraLiveInjected').forEach(x=>x.remove());page.querySelector('.empty,.liveLoadingPanel')?.remove();[...page.children].forEach(child=>{if(!child.classList.contains('veloraLiveInjected'))child.classList.add('veloraLiveOriginal')});page.classList.add('livePageEnhanced','duloLivePage');const verified=channels.filter(c=>c.health==='working'),down=channels.filter(c=>c.health==='down'),unknown=channels.filter(c=>c.health==='unknown'),good=verified;const focus=sports.focus||{mode:'channels',label:'Best live channels',events:[]};
    ...
    const scheduledEvents=(sports.events||[]).filter(ev=>{
      ...
      const delta=t-now;
      return (ev.state==='in'||(Number.isFinite(t)&&delta>=-60*60*1000&&delta<=48*60*60*1000)) && eventPlayback(ev,channels);
    }).slice(0,18);
