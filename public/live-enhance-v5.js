(() => {
  console.info('[Velora] live UI v5 loaded');
  ...
  function channelCard(ch){const b=document.createElement('button');b.className='duloChannelCard';const status=ch.health==='working'?'<span class="duloVerified">✓</span>':ch.health==='unknown'?'<span class="duloChecking">•</span>':'';b.innerHTML=`<div class="duloChannelArt"><span class="duloLiveDot">● <i>Live</i></span>${status}${ch.logo?`<img src="${esc(ch.logo)}" alt="" loading="lazy">`:`<b class="duloFallback">${esc((ch.name||'TV').slice(0,3).toUpperCase())}</b>`}<strong>▶</strong></div><div class="duloChannelMeta"><b>${esc(ch.name)}</b><span>${esc(ch.rawCategory||ch.category||'Live TV')}</span></div>`;b.addEventListener('click',()=>openPlayer(ch));return b;}
  ...
  function eventCard(ev,channels){const matched=matchEventChannel(ev,channels);const card=document.createElement(matched?'button':'div');card.className='duloEventCard'+(matched?' watchable':'');const fmt=ev.state==='in'?(ev.detail||'Live now'):(ev.date?new Date(ev.date).toLocaleString([],{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'Upcoming');card.innerHTML=`<span class="duloLeague">${ev.state==='in'?'● LIVE · ':''}${esc(ev.league||'LIVE')}</span><div class="duloMatchup"><div>${ev.away?.logo?`<img src="${esc(ev.away.logo)}" alt="">`:`<b>${esc(ev.away?.abbreviation||'A')}</b>`}</div><i>vs</i><div>${ev.home?.logo?`<img src="${esc(ev.home.logo)}" alt="">`:`<b>${esc(ev.home?.abbreviation||'H')}</b>`}</div></div><strong>${esc(ev.shortName||ev.name||'Upcoming event')}</strong><small>${esc(fmt)}</small>${matched?`<em>${ev.state==='in'?'Watch live':'Scheduled on'} ${esc(matched.name)} →</em>`:''}`;if(matched)card.addEventListener('click',()=>openPlayer(matched,ev.shortName||ev.name));else card.innerHTML+=`<em class="duloNoStream">No public stream matched yet</em>`;return card;}
